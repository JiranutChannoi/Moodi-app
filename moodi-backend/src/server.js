...
const app = express();
app.use(cors());                // ถ้าจะล็อก origin ค่อยเพิ่มทีหลัง
app.use(express.json());

// Log ทุกรีเควส (ช่วยหา 500)
app.use((req, _res, next) => {
  console.log('➡', req.method, req.url, '| ct:', req.headers['content-type'] || '-', '| body:', req.body);
  next();
});
...
const pool = new Pool({
  ...
  ssl: false, // บน localhost ให้ false
});
...
app.post('/users', async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: 'สมัครสมาชิกสำเร็จ', user: ins.rows[0] });
  } catch (e) {
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 Moodi API on http://localhost:${PORT}`);
});
